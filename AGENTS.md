...
- Hosted paid mode requires VERCEL_ENV=production, explicit HOSTED_LIVE_ENABLED=true, exact APP_ORIGIN, and Redis admission. Previews stay mock-only. The approved Redis store retains only admission metadata: a 100-attempt budget, consumed IDs, and a 90-second lease. Never initialize/reset its budget from requests or deployments. See docs/deployment.md. Local --live safeguards remain unchanged.
