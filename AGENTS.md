...
- Hosted deployment uses vercel.json Services and apps/server/src/vercel.ts; bun run build:deploy builds the game-only release. Production bundles omit the Generation lab; keep gameplay's /api/lab/profiles and event APIs.
- Hosted paid mode requires VERCEL_ENV=production, explicit HOSTED_LIVE_ENABLED=true, exact APP_ORIGIN, and per-attempt consent. Previews stay mock-only. Hosted allowance defaults to 100 in memory per instance; counts, duplicate-ID checks, and the busy slot reset on cold starts/redeployments and are not shared across instances. No database or global spending cap. See docs/deployment.md. Local --live defaults to 3 attempts.
