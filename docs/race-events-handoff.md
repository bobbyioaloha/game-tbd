...
- The main race offers at most two stars, one fresh attempt per star. A collectible or active generated event blocks the second star and its recovery timer. After the event fully expires, or after a failed attempt or missed star without an event, wait four gameplay seconds before offering the second star. Fix its horizontal position at the player's current x/z and place it `max(120 m, current fall speed × 4 s)` ahead, giving at least four seconds of approach at its spawn-time speed. Yellow voice stars belong to the player; rivals can activate only the generated object.
- `CreationAttempt` still owns one attempt; rearming it never resets the shared event. Only a full run reset clears the runtime. Every completed result stays `ready` for at least two gameplay seconds before appearing. It also waits while a previous effect remains active; at most one result waits and is placed from the player's latest position.
- Capture/transcription/generation keep separate 8/10/30-second budgets. Admission also reserves the reveal buffer and approach/effect time and the star's speaking window. Check before offering the second star, at recording, and at submission; a slow first attempt can leave no second opportunity, and later boosts can still make final placement too late. No automatic retries or encores.
- Main-race consent allows up to two fresh attempts (up to six API calls), each with a unique consent UUID. Server admission and allowance are unchanged; lab consent remains per attempt.
