...
Keep the eight-second capture limit, separate ten-second upload/transcription deadline, thirty-second generation deadline, and final placement checks. Keep all paid-mode gates, per-attempt consent and IDs, allowance limits, the shared live-request slot, schema validation, and no automatic retries. This is an initial dispatch for a collected grant, not a retry of the first attempt.
