...
The main race has a separate, default-off **Include an AI incident report when each event ends** opt-in. It permits up to two additional paid generation calls, one per completed/expired event. Voice consent still covers only its two attempts and up to six paid calls; with reporting enabled, the combined ceiling is eight paid calls plus the existing moderation requests. Report attempts consume the same allowance and busy slot, so capacity may run out before every optional report is written.
