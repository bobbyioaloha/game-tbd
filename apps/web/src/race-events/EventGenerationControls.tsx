...
import {
  raceEventFixtures, PipelineRequestSchema, type PipelineRequest, type VoiceRequest,
  type RaceEventCreation, type RaceEventPipelineEvent, type StageMetric,
} from '@sky/shared';
...
export function EventGenerationControls({onCreation,onBusy,fixturePrompt}:{onCreation:(spec:RaceEventCreation)=>void;onBusy:(busy:boolean)=>void;fixturePrompt:string}) {
  ...
  useEffect(()=>{setConsent(false);},[profileId,text,source]);
  useEffect(()=>{setText(fixturePrompt);setConsent(false);},[fixturePrompt]);
  ...
  const finish=(attempt:Attempt,message:string,spec?:RaceEventCreation)=>{
  ...
  const progress=(attempt:Attempt,event:RaceEventPipelineEvent)=>{
    if(active.current!==attempt)return;
    if(event.type==='stage')setStatus({design:'Choosing event and designing object…',geometry:'Building the object…',validation:'Validating event and geometry…'}[event.stage]);
    if(event.type==='design')setMetrics([event.metric]);
  ...
  const begin=():Attempt|undefined=>{
    ...
    active.current=attempt;setBusy(true);onBusy(true);setConsent(false);setMetrics([]);setTranscript('');setElapsed(0);return attempt;
  };
  const generate=async()=>{
    const attempt=begin();if(!attempt)return;setStatus('Starting event generation…');
    try {const spec=await raceEventClient.generate(attempt.request,attempt.controller.signal,event=>progress(attempt,event));
  ...
  const release=async()=>{
    ...
    try {
      ...
      const spec=await raceEventClient.generateVoice(recording,request,attempt.controller.signal,event=>{
        if(active.current!==attempt)return;
  ...
  return <section className="event-generation voice-lab">
    ...
    {(source==='text'||!live)&&<>
      <label>Example<select className="lab-input" aria-label="Event example" disabled={busy} value={raceEventFixtures.some(item=>item.prompt===text)?text:''} onChange={event=>setText(event.target.value)}>
        <option value="" disabled>Custom idea</option>{raceEventFixtures.map(item=><option key={item.prompt}>{item.prompt}</option>)}
      </select></label>
      {source==='text'&&<label>Your creation · 10 words maximum<input className="lab-input" aria-label="Event prompt" maxLength={200} value={text} disabled={busy} onChange={event=>setText(event.target.value)}/></label>}
    </>}
    {!live&&<p className="voice-mode-notice">Mock mode uses the selected fixture{source==='voice'?' as a simulated transcript, regardless of what you say':''}. No AI calls.</p>}
    ...
    {source==='text'?<button className="generate" disabled={busy||!available} onClick={()=>{void generate();}}>{live?'Generate event':'Run mock event pipeline'}</button>:
