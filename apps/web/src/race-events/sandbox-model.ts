import type { EventRacer, EventVector, RaceEncounter, RaceEventSnapshot, RacerEventInput } from '@sky/shared';
import { RaceEventBridge } from './bridge';
import { RaceEventRuntime } from './runtime';
import { add, subtract, scale, direction, contactTime, length } from './math';
...
export class EventSandboxModel {
  ...
  constructor(private readonly spec:RaceEncounter,triggerer:string,seed:number) {
    let lane=0;
  ...
  private scriptedSteering(racer:EventRacer,event:RaceEventSnapshot):EventVector {
    if(this.spec.version!==4||event.phase!=='active')return [0,0,0];
    if(this.spec.drill.family==='buddy') {
      const tether=event.drill?.tethers?.find(tether=>tether.racerIds.includes(racer.id));
      if(!tether)return [0,0,0];
      const partner=tether.racerIds[0]===racer.id?tether.to:tether.from;
      const delta=subtract(racer.position,partner);
      // Attempt to steer apart so the visible tether has something to resist.
      const away:EventVector=[delta[0],0,delta[2]];
      return scale(direction(length(away)>0.01?away:[tether.racerIds[0]===racer.id?-1:1,0,0]),8);
    }
    if(this.spec.drill.family==='observation'&&racer.id!=='creator') {
      // Creator demonstrates straight descent; rivals make bounded lane changes.
      // Apply steering as acceleration so inspection shoves still change their motion.
      const index=SANDBOX_RACERS.findIndex(item=>item.id===racer.id);
      const desired=9*Math.sin(event.elapsedSeconds*1.5+index*Math.PI/2);
      return [Math.max(-10,Math.min(10,(desired-racer.velocity[0])*2)),0,0];
    }
    return [0,0,0];
  }
  step(dt:number) {
    ...
    this.racers=this.racers.map(racer=>{
      ...
      const acceleration=add(input.acceleration,this.scriptedSteering(racer,event));
      const velocity=add(add(racer.velocity,scale(acceleration,dt)),input.velocityDelta);
