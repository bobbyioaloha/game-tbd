...
export function EventSandbox({spec}:{spec:RaceEventCreation}) {
  ...
  return <section className="event-sandbox" aria-label="Race event sandbox">
    ...
    </Canvas><div className="event-stage-label"><strong>{spec.displayName}</strong><span>{raceEventPreset(spec.effect.type).label} · {state.phase} · {state.remainingSeconds.toFixed(1)} s</span></div></div>
    ...
    <small>Local sandbox · scripted racers · {model.elapsed.toFixed(1)} / 15 s · {model.obstacleHits} obstacle hits. Gold debris collides; small blue fragments are visual only. Replay, pause and inspection make no API calls.</small>
  </section>;
}
