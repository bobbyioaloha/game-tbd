...
export function RaceEventRenderer({events,debug=false,modelScale=1,modelDiameter,modelTilt=[0,0],pickupContactRadius=RACE_EVENT_LIMITS.collectibleRadius+RACE_EVENT_LIMITS.racerRadius}:
  ...
  useFrame(()=>{
    ...
    if(model.current) {
      model.current.visible=state.instance?.spec.version!==4||state.phase==='collectible';
    ...
    const waveRadius=state.instance?.spec.version===3&&state.instance.spec.effect.type==='repulsionBurst'&&!debug?34*Math.min(1,state.elapsedSeconds/0.45):displayRadius;
    if(field.current){field.current.visible=state.phase==='active'&&state.instance?.spec.version===3&&state.radius>0;field.current.scale.setScalar(waveRadius);}
    if(outline.current){outline.current.visible=state.phase==='collectible'||(state.instance?.spec.version===3&&state.radius>0);outline.current.scale.setScalar(state.phase==='collectible'?pickupContactRadius:waveRadius);
      if(state.instance)(outline.current.material as MeshBasicMaterial).color.set(state.phase==='collectible'?'#ffe16c':state.instance.spec.version===3?tint[state.instance.spec.effect.type]:'#76e0da');}
  ...
  return <>
    {spec.version===4&&<DrillRenderer events={events} spec={spec} debug={debug}/>}
    <group ref={root}>
    <group ref={model}><group ref={content}><PowerUpModel spec={spec}/></group></group>
    <mesh ref={field}>
      <sphereGeometry args={[1,32,20]}/>
      <meshBasicMaterial color={spec.version===3?tint[spec.effect.type]:'#76e0da'} wireframe={debug} transparent opacity={debug?0.18:0.17} side={DoubleSide} depthWrite={false}/>
    </mesh>
