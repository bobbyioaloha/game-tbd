...
export function RaceEventRenderer({events,debug=false,modelScale=1,modelTilt=[0,0],pickupContactRadius=RACE_EVENT_LIMITS.collectibleRadius+RACE_EVENT_LIMITS.racerRadius}:
  {events:RaceEventPort;debug?:boolean;modelScale?:number;modelTilt?:readonly [number,number];pickupContactRadius?:number}) {
  const initial=events.getSnapshot();
  const root=useRef<Group>(null),model=useRef<Group>(null),field=useRef<Mesh>(null),pickup=useRef<Mesh>(null),outline=useRef<Mesh>(null),debris=useRef<InstancedMesh>(null);
  ...
  return <group ref={root}>
    ...
    <mesh ref={pickup} visible={false}><sphereGeometry args={[pickupContactRadius,16,12]}/><meshBasicMaterial wireframe color="#ffda6e"/></mesh>
    <mesh ref={outline} rotation={[-Math.PI/2,0,0]}>
      <ringGeometry args={[0.94,1,64]}/><meshBasicMaterial color={tint[spec.effect.type]} transparent opacity={0.65} side={DoubleSide} depthWrite={false}/>
    </mesh>
