...
export function planRival(race:PracticeRace,racer:Racer){
  ...
  for(const option of options){
    ...
    const moving=Math.hypot(x-p[0],z-p[2])>1;
    const inspected=moving&&(drill?.observers??[]).some(observer=>(observer.watching||observer.warning)&&[0,0.35,0.7].some(t=>
      insideObservationCone([p[0]+(x-p[0])*t,p[1]+(end[1]-p[1])*t,p[2]+(z-p[2])*t],observer)));
    const score=reward-(collisions+equipmentContacts)*(26-risk*8)-(inspected?22:0)-Math.hypot(x-p[0],z-p[2])*0.06;
