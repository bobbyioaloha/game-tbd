...
export function Preview({angle,zoom,pose,paused,chase,inGame=false,loop=false,character='greg'}:{angle:number;zoom:number;pose:GregPose;paused:boolean;chase:boolean;inGame?:boolean;loop?:boolean;character?:DinosaurCharacter}) {
  ...
  return <>
    ...
    <group ref={rig}><DinosaurModel character={character} pose={pose} loop={loop} paused={paused} wind={pose==='Stand'||pose==='Reach'||pose==='Checklist'?undefined:()=>({time:windTime.current,speed:pose==='Brake'?12:45})}/></group>
  ...
  return <main className="personnel-page">
    ...
    <div className="personnel-layout">
      <section className="personnel-preview" aria-label={`Interactive 3D preview of ${person.name}`}>
        ...
        <Canvas dpr={[0.75,1]} gl={{antialias:false}} camera={{position:[0,2.6,8],fov:42}} fallback={<p>3D preview requires WebGL. Greg can still be selected below.</p>}>
          <Preview character={inspected} key={inspected+take} angle={angle} zoom={zoom} pose={viewer?pose:(person.introPose??'Stand')} loop={!viewer||pose==='Checklist'} paused={paused} chase={chase}/>
        </Canvas>
      ...
      <section className="personnel-file" aria-label="Personnel roster">
        ...
        <div className="personnel-roster">{CHARACTERS.map((character,index)=><button key={character.id} disabled={!character.model} aria-pressed={character.id===person.id} className={character.id===person.id?'assigned':''} onClick={()=>{if(character.model){setInspected(character.model);setPose(character.introPose??'Stand');setPaused(false);setTake(value=>value+1);}}}>
          <span className="roster-index" style={{borderColor:character.color}}>{String(index+1).padStart(2,'0')}</span>
          <span><strong>{character.name}</strong><small>{character.species}</small></span><span className="roster-state">{viewer&&character.id===person.id?'INSPECTING':character.ready?'PLAYER':character.model?'RIVAL':'PLACEHOLDER'}</span>
        </button>)}</div>
        {!viewer&&<button className="report-button" onClick={onStart}>{inspected==='greg'?'Report for exercise →':'Report as Greg →'}</button>}
        <p className="personnel-note">{viewer?'Inspect the same character models used in the race.':'Linda is equipped. Steve and Susan are awaiting equipment fitting.'}</p>
      </section>
    </div>
    <section className="inspection-controls" aria-label="Model inspection controls">
      ...
      <div><label htmlFor="greg-pose">Procedure preview</label><select id="greg-pose" value={pose} onChange={event=>{setPose(event.target.value as GregPose);if(event.target.value==='Reach')setAngle(150);setPaused(false);setTake(value=>value+1);}}>
        {(viewer?['Stand','Dive','Reach','Brake','Bank left','Bank right','Impact',...(inspected==='linda'?['Checklist']:[])]:['Stand','Reach']).map(value=><option key={value} value={value}>{value==='Reach'?'Check ripcord reach':value}</option>)}
      </select></div>
