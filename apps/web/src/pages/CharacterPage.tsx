...
export function Preview({angle,zoom,pose,paused,chase,inGame=false,loop=false,character='greg'}:{angle:number;zoom:number;pose:GregPose;paused:boolean;chase:boolean;inGame?:boolean;loop?:boolean;character?:DinosaurCharacter}) {
  ...
  return <>
    ...
    <group ref={rig}><DinosaurModel character={character} pose={pose} loop={loop} paused={paused} wind={pose==='Stand'||pose==='Reach'||pose==='Checklist'||pose==='Diagnostics'?undefined:()=>({time:windTime.current,speed:pose==='Brake'?12:45})}/></group>
  ...
  return <main className="personnel-page">
    ...
    <div className="personnel-layout">
      <section className="personnel-preview" aria-label={`Interactive 3D preview of ${person.name}`}>
        ...
        <Canvas dpr={[0.75,1]} gl={{antialias:false}} camera={{position:[0,2.6,8],fov:42}} fallback={<p>3D preview requires WebGL. Greg can still be selected below.</p>}>
          <Preview character={inspected} key={inspected+take} angle={angle} zoom={zoom} pose={viewer?pose:(person.introPose??'Stand')} loop={!viewer||pose==='Checklist'||pose==='Diagnostics'} paused={paused} chase={chase}/>
        </Canvas>
      ...
      <section className="personnel-file" aria-label="Personnel roster">
        ...
        <p className="personnel-note">{viewer?'Inspect the same character models used in the race.':'Linda and Steve are equipped. Susan is awaiting equipment fitting.'}</p>
      </section>
    </div>
    <section className="inspection-controls" aria-label="Model inspection controls">
      ...
      <div><label htmlFor="greg-pose">Procedure preview</label><select id="greg-pose" value={pose} onChange={event=>{setPose(event.target.value as GregPose);setPaused(false);setTake(value=>value+1);}}>
        {(viewer?['Stand','Dive','Reach','Brake','Bank left','Bank right','Impact',...(inspected==='linda'?['Checklist']:inspected==='steve'?['Diagnostics']:[])]:['Stand','Reach']).map(value=><option key={value} value={value}>{value==='Reach'?'Check ripcord reach':value}</option>)}
      </select></div>
